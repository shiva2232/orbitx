...
type StunClient struct {
	...
	sendRequestFunc   func(packet []byte) error
}
...
func (c *StunClient) listenLoop() {
	...
	for {
		...
		default:
			...
			c.mu.Lock()
			changed := c.publicAddress == nil || !c.publicAddress.Equal(res.Address)
...
func (c *StunClient) Start(conn *net.UDPConn, host string, port int, callback func(net.IP, int, bool)) error {
...
func (c *StunClient) StartShared(sendRequest func([]byte) error, callback func(net.IP, int, bool)) error {
	...
	c.mu.Lock()
	c.sendRequestFunc = sendRequest
	c.onEndpointChanged = callback
	c.stopChan = make(chan struct{})
	c.mu.Unlock()

	err = c.sendBindingRequestShared(addr)
	if err != nil {
		return err
	}
	...
	go func() {
		...
		for {
			select {
			case <-ticker.C:
				c.sendBindingRequestShared(addr)
			case <-c.stopChan:
				return
			}
		}
...
func (c *StunClient) sendBindingRequestShared(addr *net.UDPAddr) error {
	tid := make([]byte, 12)
	if _, err := rand.Read(tid); err != nil {
		return err
	}
...
func (c *StunClient) ProcessPacket(buf []byte) {
	res, err := c.parse(buf)
	if err != nil {
		return
	}
